...
function onMeta(meta) {
  ...
  document.getElementById('lobbyTitle').textContent = meta.title || 'Copa de Cumple';
...
function onQuestions(q) {
  questions = q || [];
  document.getElementById('questionCount').textContent = `${questions.length} preguntas cargadas`;
  // pre-cargar en el editor
  const editor = document.getElementById('questionsEditor');
  if (editor && questions.length) {
    editor.value = questions.map(q =>
      `${q.text} | ${q.options.join(' | ')} | ${LABELS[q.correct]}`
    ).join('\n');
  }
  updateStartBtn();
}
...
function renderPhase(meta) {
  ...
  if (gamePhase === 'lobby')    { document.getElementById('phaseLobby').classList.remove('hidden'); }
  if (gamePhase === 'question') { document.getElementById('phaseQuestion').classList.remove('hidden'); renderQuestion(); }
  if (gamePhase === 'results')  { document.getElementById('phaseResults').classList.remove('hidden'); renderResults(); }
  if (gamePhase === 'podio')    { document.getElementById('phasePodio').classList.remove('hidden'); renderPodio(); }
}
...
function renderPlayers() {
  ...
  el.innerHTML = list.length
    ? list.map(p => `<div class="player-chip">${p.avatar} ${escHtml(p.name)}</div>`).join('')
    : '<span style="color:var(--dim);font-size:17px;">Esperando jugadores...</span>';
  const cntEl = document.getElementById('playerCount');
  if (cntEl) cntEl.textContent = `${list.length} jugador${list.length===1?'':'es'}`;
}
...
function renderPartialLb(containerId) {
  ...
  el.innerHTML = sorted.map((p,i) => {
    ...
    const mark = pts > 0 ? `<span style="color:var(--lime);">+${pts}</span>` : '';
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    timerLeft--;
    updateTimerUI();
    if (timerLeft <= 0) { clearInterval(timerInterval); closeQuestion(); }
  }, 1000);
}
...
function updateTimerUI() {
  ...
  bar.style.width = pct + '%';
  el.className = 'timer-big' + (timerLeft <= 5 ? ' danger' : timerLeft <= 10 ? ' warning' : '');
  bar.className = 'bar-fill'  + (timerLeft <= 5 ? ' danger' : timerLeft <= 10 ? ' warning' : '');
}
...
function closeQuestion() {
  ...
  Object.entries(answersThisRound).forEach(([uid, ans]) => {
    if (ans.option === q.correct) {
      ...
      const elapsed  = Math.min(QUESTION_TIME * 1000, ans.ts - (ans.questionStartTs || ans.ts));
...
async function nextQuestion() {
  ...
  if (next >= questions.length) {
    ...
    await dbUpdate('party/meta', { phase:'question', questionIndex: next, questionStartTs: Date.now() });
  }
}
...
function showCountdown(n, cb) {
  ...
  ov.textContent = n;
  if (n <= 0) { ov.classList.add('hidden'); cb(); return; }
...
document.getElementById('startGameBtn').addEventListener('click', async () => {
  ...
  await dbUpdate('party/meta', { phase:'question', questionIndex:0, questionStartTs: Date.now() });
  await dbRemove('party/answers');
  showCountdown(3, () => {});
});
...
document.getElementById('saveQuestionsBtn').addEventListener('click', () => {
  ...
  const parsed = raw.split('\n').filter(Boolean).map(line => {
    ...
    return { text: parts[0], options: [parts[1], parts[2], parts[3], parts[4]], correct };
  }).filter(Boolean);
  if (!parsed.length) { alert('No se encontraron preguntas válidas. Revisá el formato.'); return; }
